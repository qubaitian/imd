...
const names = (page) => page.locator(".session-layout > .app-shell .breadcrumb > span:first-of-type");
...
test.beforeEach(async ({ page, request }) => {
  let documents = (await (await request.get(session + "/api/session", { headers })).json()).documents;
...
test("moves panels across neighbors and restores session order on reload", async ({ page, request }) => {
  ...
  await expect(names(page)).toHaveText(["c.md", "a.md", "b.md", "notes.txt"]);
  ...
  await expect(names(page)).toHaveText(["a.md", "b.md", "notes.txt", "c.md"]);
  await expect(page).toHaveTitle("a.md | b.md | notes.txt | c.md · IMD");
  await drag(page, panel(page, "notes.txt"), panel(page, "a.md"));
  await expect(names(page)).toHaveText(["notes.txt", "a.md", "b.md", "c.md"]);
});
...
test("keeps the editor draft and scroll position when a panel moves", async ({ page }) => {
  ...
  await expect(names(page)).toHaveText(["b.md", "c.md", "a.md", "notes.txt"]);
...
test("cancels a drag and keeps title bar buttons clickable", async ({ page }) => {
  ...
  await page.mouse.up();
  await expect(names(page)).toHaveText(["a.md", "b.md", "c.md", "notes.txt"]);
...
test("moves repeated file panels without exchanging their drafts", async ({ page }) => {
  ...
  await expect(names(page)).toHaveText(["a.md", "a.md", "b.md", "c.md", "notes.txt"]);
